  ...
  return new Promise(function (resolve, reject) {
    ...
    User.find({
      ...
      if (users.length === 0) {
        ...
        reject(new Error('Failed due to local account already exists: ' +
          user.username));
      }
    });
  });
  ...
  return function (user) {
    return new Promise(function (resolve, reject) {
      if (seedOptions.logResults) {
        console.log(chalk.bold.red('Database Seeding:\t\t\tLocal ' + user
          .username + ' added with password set to ' + password));
      }
      resolve();
    });
  };
...
function seedTheUser(user) {
  return function (password) {
    return new Promise(function (resolve, reject) {
      ...
      if (user.username === seedOptions.seedAdmin.username && process.env
        .NODE_ENV === 'production') {

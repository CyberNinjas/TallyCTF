  ...
  for(var i = 0; i < max; ++i) {
    ...
    if(i === max - 1)
      return res.status(400)
        .send({ message: 'Invalid User to add' });
  }
  // Try to add the user, error if alreay on a team
  if(!exports.addTeamToUser(user, team))
    return res.status(400)
      .send({ message: 'User is already a member of a team!' });
...
exports.removeMember = function (req, res) {
  ...
  if(user.team.toString() !== team._id.toString()) {
    return res
      .status(400)
      .send({ message: 'User is not in specified team!' });
  }
  // Update the team
...
exports.delete = function (req, res) {
  ...
  if((user.roles.indexOf('admin') === -1) && (!user || team.teamCaptain._id.toString() !== user._id.toString())) {
    return res
      .status(503)
      .send({ message: 'Not authorized to delete this team!' });
  }
  ...
  Team
    ...
    .exec(function (err, team) {
      if(err) {
        ...
        return res
          .status(404)
          .send({ message: 'No team with that identifier has been found' });
      }
      req.team = team;
      next();
    });

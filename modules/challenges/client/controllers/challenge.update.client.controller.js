  ...
  function ($scope, $stateParams, $location, Authentication, $http, filterFilter, Challenges) {
    ...
    $scope.challenge.$promise.then(function () { //Find and select the values returned for the object.
      $scope.challenge.type = $scope.challengeTypes.filter(function (obj) {
        return obj.name === $scope.challenge.challenge_type;
      })[0];
      $scope.updateType();
      $scope.challenge.format = $scope.challenge.type.formats.filter(function (obj) {
        return obj.name === $scope.challenge.challenge_format;
      })[0];
      $scope.updateFormat();
    });

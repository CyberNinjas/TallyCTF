...
gulp.task('cssmin', function () {
  return gulp.src(defaultAssets.client.css).pipe(plugins.cssmin()).pipe(plugins.concat('application.min.css')).pipe(gulp.dest('public/dist'));
});
// Sass task
...
gulp.task('mocha', function (done) {
  ...
  mongoose.connect(function () {
    mongoose.loadModels();
    // Run the tests
    gulp.src(testAssets.tests.server).pipe(plugins.mocha({
      reporter: 'spec',
      timeout: 10000
...
gulp.task('ngdocs', [], function () {
  ...
  return gulp.src(assets).pipe(gulpDocs.process(options)).pipe(gulp.dest('./docs'));
});
